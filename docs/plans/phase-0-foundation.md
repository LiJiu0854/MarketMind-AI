...
- [x] 编写测试，证明缺少请求头时会生成有效 UUID、传入 ID 时会原样保留、请求状态包含相同值，而且响应头与其一致。
- [x] 只运行中间件测试，看到测试因中间件模块不存在而失败。
- [x] 实现让测试通过所需的最小中间件。
- [x] 再次运行聚焦测试并看到通过。
- [x] 运行 Ruff、mypy 和完整测试。
- [x] 完成任务 4 本人练习并得到用户确认。
- [x] 使用 `feat: add request id middleware` 创建提交。
...
- [x] 编写 API 测试，覆盖存活检查的精确响应内容、200 状态码、请求 ID 响应头和 `/docs` 的 200 状态码。
- [x] 只运行健康检查 API 测试，看到测试因应用模块不存在而失败。
- [x] 添加响应模型、健康检查路由和组装请求 ID 中间件的应用工厂。
- [x] 再次运行聚焦 API 测试并看到通过。
- [x] 在 8010 端口启动应用，手工验证存活检查和接口文档端点。
- [x] 运行 Ruff、mypy 和完整测试。
- [x] 任务 5 不重复安排本人练习；统一进入任务 6 的阶段综合学习门禁。
- [x] 使用 `feat: add FastAPI health vertical slice` 创建提交。
...
- [x] 已于 2026-08-26 收到本人明确验收；验收前未合并，也未创建标签。
